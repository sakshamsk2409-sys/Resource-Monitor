...
class ProcessManagerPage(CarbonFiberBackground):
    ...
    def setup_ui(self):
        ...
        subtitle = QLabel("Monitor Running Tasks, Process Telemetry & Manage System Resources")
        ...
        self.refresh_btn.setStyleSheet(self.get_btn_style("#26a69a"))
        self.refresh_btn.clicked.connect(self.refresh_processes)
        controls_layout.addWidget(self.refresh_btn)

        self.terminate_btn = QPushButton("⏹ End Task")
        self.terminate_btn.setStyleSheet(self.get_btn_style("#ff8f00"))
        ...
        self.count_lbl = QLabel("Total Processes: 0")
        self.count_lbl.setStyleSheet("color: #ffaa00; font-size: 11px; font-weight: bold;")

        self.mem_summary_lbl = QLabel("Monitored RAM: 0 MB")
        self.mem_summary_lbl.setStyleSheet("color: #ab47bc; font-size: 11px; font-weight: bold;")

        summary_layout.addWidget(self.count_lbl)
        summary_layout.addStretch()
        summary_layout.addWidget(self.mem_summary_lbl)
        ...
        self.table.setHorizontalHeaderLabels([
            "PID", "Process Name", "Status", "CPU %", "RAM (MB)", "RAM %", "User / Owner"
        ])
        ...
        for proc in psutil.process_iter(["pid","name","status","cpu_percent","memory_info","username",]):
            ...
            try:
                ...
                try:
                    ...
                    mem_pct = (mem_info.rss / total_ram * 100) if mem_info and total_ram else 0.0
                except Exception:
                    rss_mb = 0.0
                    mem_pct = 0.0
    ...
    def filter_processes(self):
        ...
        for item in self.processes_data:
            if not query or query in item["name"].lower() or query in str(item["pid"]):
        ...
        total_rss = sum(x["rss_mb"] for x in filtered)

        for row, item in enumerate(filtered):
            # PID
            pid_item = QTableWidgetItem()
            pid_item.setData(Qt.DisplayRole, item["pid"])
            ...
            st_color = "#55d66f" if item["status"] in ("running", "active") else "#aaaaaa"
            st_item.setForeground(QColor(st_color))

            # CPU %
            cpu_item = QTableWidgetItem()
            cpu_item.setData(Qt.DisplayRole, round(item["cpu"], 1))
            cpu_color = "#e53935" if item["cpu"] > 20 else "#cccccc"
            cpu_item.setForeground(QColor(cpu_color))

            # RAM MB
            ram_item = QTableWidgetItem()
            ram_item.setData(Qt.DisplayRole, round(item["rss_mb"], 1))
            ram_item.setForeground(QColor("#ab47bc"))

            # RAM %
            ram_pct_item = QTableWidgetItem()
            ram_pct_item.setData(Qt.DisplayRole, round(item["mem_pct"], 1))
            ram_pct_item.setForeground(QColor("#8bc34a"))

            # User
            user_item = QTableWidgetItem(item["user"])
            user_item.setForeground(QColor("#888888"))
            ...
            self.table.setItem(row, 5, ram_pct_item)
            self.table.setItem(row, 6, user_item)

        self.table.setSortingEnabled(True)
        self.count_lbl.setText(f"Total Processes: {len(filtered)}")
        self.mem_summary_lbl.setText(f"Monitored RAM: {total_rss:.1f} MB")
    ...
    def get_selected_pid_name(self):
        ...
        if pid_item and name_item:
            return int(pid_item.text()), name_item.text()
        return None, None

    def terminate_selected_process(self):
        pid, name = self.get_selected_pid_name()
        if not pid:
        ...
        reply = QMessageBox.question(
            self,
            "Confirm End Task",
            f"Are you sure you want to end task for process '{name}' (PID: {pid})?",
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                proc.terminate()
                QMessageBox.information(self, "Task Terminated", f"Process '{name}' (PID: {pid}) termination signal sent.")
                self.refresh_processes()
            except psutil.AccessDenied:
                QMessageBox.warning(self, "Access Denied", f"Permission denied to terminate process '{name}' (PID: {pid}). Run application as Administrator.")
            except psutil.NoSuchProcess:
                QMessageBox.information(self, "Process Ended", f"Process '{name}' (PID: {pid}) has already ended.")
                self.refresh_processes()
    ...
    def kill_selected_process(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                proc = psutil.Process(pid)
                proc.kill()
                QMessageBox.information(self, "Force Killed", f"Process '{name}' (PID: {pid}) was forcefully killed.")
                self.refresh_processes()
            except psutil.AccessDenied:
                QMessageBox.warning(self, "Access Denied", f"Permission denied to kill process '{name}' (PID: {pid}). Run application as Administrator.")
            except psutil.NoSuchProcess:
                QMessageBox.information(self, "Process Ended", f"Process '{name}' (PID: {pid}) has already ended.")
                self.refresh_processes()
            except Exception as e:
